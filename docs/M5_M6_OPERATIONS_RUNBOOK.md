...
- 읽기 전용 preflight 결과가 `FRESH`일 때만 V001~V013을 적용한다. Neon이 만든
...
5. 운영 DB는 `ops/db/roles` 순서로 준비한 뒤 고정 image tag에서
   `docker compose -f compose.migration.yaml run --rm migration`을 한 번 실행해 V013까지
...
9. Caddy는 외부 `X-Forwarded-For`와 내부 token header를 upstream에서 덮어쓴다.
   ...
   publish하지 않는다.
10. 관리자 운영 화면에 버전·시작 시각·세 flag·V013 상태가 표시되고 URL·비밀값이
   없는지 확인한다.
