...
로컬 앱은 자동 마감 스케줄러도 활성화한다. 출석일에 저장된 정책별
`finalization_due_at`에 맞춰 동적으로 실행하고, 재기동하면 DB에서 미마감 작업을
복구한다. 실패하면 1·2·4·8·16분 간격으로 다섯 번 재시도하며 미기록 대상자는
`ABSENT`, 날짜는 `FINALIZED`로 저장한다. 개인 출석 통계는 이 공식 마감이 끝난
날짜만 분모에 포함한다.
