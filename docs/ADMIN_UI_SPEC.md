...
|---|---|
| 이름 | 필수, 공백 제거, 1~255자 |
| 생년월일 | 신규 등록과 활성 교사 기본정보 수정 모두 필수; 정확한 날짜만 허용하고 미래 날짜 거부; 생일 관리와 만 나이 계산에 사용 |
| 전화번호 | 선택, 빈 문자열은 `NULL`, 255자 이하 |
| 카드 연결 | 선택, 카드 등록함의 자기 부서 최근 미등록 또는 재사용 가능 카드 태깅만 선택 |

나이는 서버가 저장된 생년월일과 조회일을 기준으로 계산한다. `birth`가 없는 레거시 교사는 생년월일과 계산 나이를 `미상`으로 표시하고, 관리자가 확인한 정확한 날짜로 정정하기 전까지 기존 정수 `age`에서 생년월일을 추정하지 않는다. 레거시 `member.card_uid`는 입력·수정하지 않으며 카드 lifecycle 서비스만 사용한다.

`카드 없이 교사 추가`와 `선택한 카드와 함께 추가`를 명확히 구분한다. 카드와 함께 저장하면 교사, 활성 소속, 카드, 활성 assignment와 감사 로그가 모두 성공하거나 모두 rollback되어야 한다.

상세 화면의 `수정` 버튼은 같은 URL의 명시적인 수정 모드로 전환한다. 이 모드에서 이름·연락처·생년월일을 갱신하고 카드 연결·교체·종료 또는 부서 제외를 수행한다. 생년월일이 없는 레거시 활성 교사가 이름이나 연락처를 수정하려면 확인된 정확한 생년월일을 같은 요청에 입력해야 한다. 저장 후에는 읽기 상세 화면으로 돌아간다. 하드웨어 태깅으로 연결할 때는 원본 UID를 표시하지 않는 카드 등록함으로 이동한다.

교사 기본정보 수정 감사에는 작업자·작업 시각·대상과 실제로 바뀐 필드명만 남긴다. 이름·연락처·생년월일의 원문 before/after는 저장하지 않으며 이 감사 행에는 2년 보유기간을 적용하고 별도 5년 기본정보 이력을 만들지 않는다.
...
- `UI-TEACHER-FORM-04`: 신규 등록과 활성 교사 기본정보 수정은 정확한 생년월일이 없거나 미래 날짜이면 거부된다. 레거시 결측값은 임의로 보정하지 않고 확인 전까지 `미상`으로 표시한다.
- `UI-TEACHER-FORM-05`: 기본정보 수정 감사에는 실제 변경 필드명만 있고 이름·연락처·생년월일의 원문 before/after는 없다.
...
- 이미 등록된 날짜 중 태깅 시작 전이고 출석 기록이 하나도 없어 자동 제외되는 날짜와 건수
- 과거·이미 시작됐거나 출석 기록이 하나라도 있어 자동 변경하지 않는 날짜
- 나중에 교사를 다시 추가해도 기존 미래 날짜에는 자동 복원되지 않음

| 필드 | 검증 |
|---|---|
| 제외 사유 | 필수, 1~500자 |
| 카드 처리 | 활성 카드가 있으면 `AVAILABLE`, `LOST`, `RETIRED` 중 필수 |
| 자동 제외 대상 | `SCHEDULED`, 태깅 시작 전, 날짜 전체 출석 기록 0건인 모든 대상 날짜와 건수 표시 |
| 최종 확인 | `미래 출석일 N건에서도 제외됩니다.` 영향을 확인하는 checkbox |

관리자는 미래 날짜를 개별 선택하지 않는다. 확인 후 서버가 후보 전체를 다시 계산하고 소속·카드 종료와 같은 트랜잭션에서 `is_target=false`로 변경한다. 화면을 본 뒤 후보 건수가 달라졌거나 날짜 상태·시작 시각·출석 기록이 바뀌면 전체를 rollback하고 최신 영향 목록을 다시 보여준다. 교사를 다시 소속에 추가해도 기존 `is_target=false` 행은 자동 복원하지 않는다.

**인수 기준**

- `UI-EXCLUDE-01`: 부서 제외는 `member`, 과거 소속, 대상자, 출석과 카드 이력을 삭제하지 않는다.
- `UI-EXCLUDE-02`: 소속 종료, 카드 assignment 종료·상태 변경, 조건을 만족하는 미래 대상 전체 제외와 감사 기록은 원자적으로 처리된다.
- `UI-EXCLUDE-03`: 과거·이미 시작한 날짜와 출석 기록이 하나라도 있는 날짜의 대상자·출석은 자동 변경하지 않는다.
- `UI-EXCLUDE-04`: 처리 관리자는 인증 세션에서 결정되고 빈 사유는 저장되지 않는다.
- `UI-EXCLUDE-05`: 재가입해도 기존 미래 날짜의 비활성 대상자를 자동 복원하지 않는다.
...
- `finalization_due_at` 전의 `SCHEDULED`는 `예정` 또는 `진행일`, 마감 시각이 지난 `SCHEDULED`는 `마감 지연` 경고다.
...
- `UI-DAY-LIST-02`: 마감 시각이 지난 미마감 날짜를 정상 완료로 보이지 않고 운영 경고를 표시한다.
...
| 필드 | 검증 |
|---|---|
| 시작 날짜 | 서버 기준 오늘 또는 미래 |
| 적용 정책 | 자기 부서의 `PUBLISHED` 버전만 선택 |
| 반복 | 없음, N일, N주, N개월, N년마다 중 하나 |
| 반복 종료일 | 반복일 때 필수, 시작 날짜부터 최대 5년 |

`N주마다`는 요일을 하나 이상 선택하고, `N개월마다`는 날짜를 하나 이상 선택한다. 해당 월에 없는 날짜(예: 2월 30일)는 건너뛴다. `N년마다`는 반복할 월·일을 선택한다. 반복 없음은 시작 날짜 한 건만 만든다.

오늘 날짜는 선택 정책의 태깅 시작 시각 전까지만 등록할 수 있다. form에는 현재 활성 교사 수와 `등록 시점의 활성 교사가 기본 대상자로 복사됩니다.`를 표시한다. 명단은 생성 후 상세 화면에서 조정한다.

등록 성공 시 각 새 날짜와 대상자 snapshot이 한 트랜잭션으로 생성된다. 한 요청에서 생성할 수 있는 날짜는 최대 366건이며, 이를 넘는 반복 계획은 부서 잠금을 얻기 전에 거부한다. 반복 생성에서 같은 부서·날짜가 이미 있으면 그 날짜와 기존 대상자 snapshot은 건드리지 않고 건너뛴다. 반복 없음에서 같은 날짜를 다시 등록하면 오류로 처리한다.

반복 입력은 JavaScript 없이도 모두 제출할 수 있어야 한다. JavaScript는 선택하지 않은 입력을 숨기는 보조 기능으로만 사용한다. 서버 검증이 실패하면 시작일·종료일·정책·반복 방식·간격과 세부 선택을 그대로 복원하고 사용자용 한국어 오류를 표시한다.
...
- `UI-DAY-CREATE-04`: 반복 계획은 시작 날짜부터 5년·한 요청 366건을 넘지 않으며, 생성된 날짜와 건너뛴 기존 날짜 수를 알린다.
- `UI-DAY-CREATE-05`: JavaScript가 없어도 반복 날짜를 등록할 수 있고, 서버 검증 실패 뒤 모든 입력이 복원된다.
...
| 자동 마감 | 마감 시각 경과·미마감 날짜 수, 마지막 성공·실패 시각과 상관 ID |
...
- `UI-OPS-04`: 마감 시각 경과·미마감 날짜 경고는 보여주지만 시스템 관리자 우회 마감 버튼은 제공하지 않는다.
