...
| 비밀번호 | 작업별 | 12~64 Unicode code point이면서 UTF-8 72 byte 이하 |

비밀번호에 대문자·소문자·숫자·특수문자 조합을 기계적으로 강제하지 않는다. 흔한·유출 비밀번호 목록 검사는 MVP 이후에 추가한다. 반복 로그인 제한과 예외 처리는 `SECURITY_MATRIX.md`를 단일 기준으로 사용하며, HTML 속성만 믿지 않고 서버에서 동일하게 검증한다.
...
| 새 비밀번호 | 보안 명세의 길이·UTF-8 byte 상한 적용 |
...
계정 생성 필드는 사용자명과 `SYSTEM_ADMIN` 부여 여부다. 부서 권한은 생성 후 별도 작업으로 부여한다. 시스템 관리자는 계정을 먼저 만든 뒤 회원가입 초대 토큰을 발급하며, 초대받지 않은 사용자가 직접 계정을 만드는 공개 회원가입은 제공하지 않는다. 발급 화면은 원문 token이 포함된 링크를 한 번만 표시하고 관리자가 복사해 승인된 1:1 메신저로 직접 전달한다. 애플리케이션이 메신저 계정·연락처를 저장하거나 자동 발송하지 않는다. 초대받은 사용자의 비밀번호 설정을 위해 평문 임시 비밀번호를 관리자가 저장·메일 전송하는 방식도 사용하지 않는다.
...
V002 migration과 계정 상태·token 제약의 PostgreSQL 부정 테스트가 완료되기 전에는 일반 계정의 웹 생성·회원가입 초대·재설정 command를 활성화하거나 `회원가입 초대 완료`, `비밀번호 재설정 완료`를 표시해서는 안 된다. 이 DB 출시 gate와 별개로 운영 공개 base URL·HTTPS 정책은 배포 전에 승인되어야 한다. 평문 임시 비밀번호를 DB·메일·감사 로그에 보관하는 임시 구현도 금지한다.
...
비밀번호 재설정 화면은 `RESET` 목적의 1회용 링크를 발급 직후 한 번만 표시하고, 관리자가 복사해 승인된 1:1 메신저로 직접 전달하도록 안내한다. token 없는 별도 reset 경로를 대안으로 두지 않는다. 원문 token과 비밀번호를 flash message, application log, 이메일과 감사 before/after에 넣지 않는다. V002 migration 또는 필수 DB 테스트가 완료되지 않았다면 버튼 대신 `안전한 재설정 절차가 아직 구성되지 않았습니다.`를 표시하고 쓰기 endpoint도 제공하지 않는다.
...
4. 개인정보·감사·태깅 이력 보유기간
5. 백업 작업 상태를 운영 화면에 전달할 source와 기한 초과 기준
6. scheduler 실행 주기와 운영 경고가 되는 미마감 허용 시간

위 결정·현장 확인이 완료되지 않았어도 임의의 성공 상태나 보안 효과를 화면에 표시하지 않는다.

흔한·유출 비밀번호 금지 목록의 출처와 갱신 책임은 MVP 이후 보안 강화 단계에서 결정한다.
