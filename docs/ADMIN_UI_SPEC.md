...
등록 성공 시 각 새 날짜와 대상자 snapshot이 한 트랜잭션으로 생성된다. 반복 생성에서 같은 부서·날짜가 이미 있으면 그 날짜와 기존 대상자 snapshot은 건드리지 않고 건너뛴다. 반복 없음에서 같은 날짜를 다시 등록하면 오류로 처리한다.
...
- `UI-DAY-CREATE-04`: 반복 계획은 시작 날짜부터 5년을 넘지 않으며, 생성된 날짜와 건너뛴 기존 날짜 수를 알린다.
