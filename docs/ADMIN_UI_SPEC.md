...
계정 생성 필드는 사용자명과 `SYSTEM_ADMIN` 부여 여부다. 부서 권한은 생성 후 별도 작업으로 부여한다. 최초 비밀번호 설정은 평문 임시 비밀번호를 관리자가 저장·메일 전송하는 방식으로 구현하지 않는다.

현재 목표 스키마의 `account.password_hash`는 필수이고 상태는 `ACTIVE`·`DISABLED`뿐이다. 일회용 설정·재설정 token의 hash·만료·사용 시각, `PENDING_SETUP` 또는 `must_change_password` 상태는 없다. 따라서 일반 계정의 웹 생성·최초 설정·비밀번호 재설정 UI를 운영 가능 상태로 만들기 전에 만료되는 1회용 token을 hash로 저장하는 별도 모델을 보안·DB 문서에서 확정하고 필요한 migration을 추가해야 한다.

승인된 운영자 CLI에서 사용자가 비밀번호를 즉시 입력하는 절차는 fresh DB의 **최초 `SYSTEM_ADMIN` 1회 bootstrap 전용**이다. bootstrap을 닫은 뒤의 일반 계정 생성·최초 설정·reset 대안으로 재사용하지 않는다.

token 선결 조건이 완료되기 전에는 일반 계정의 웹 생성·설정·재설정 command를 활성화하거나 `계정 생성 완료`, `비밀번호 재설정 완료`를 표시해서는 안 된다. 평문 임시 비밀번호를 DB·메일·감사 로그에 보관하는 임시 구현도 금지한다.
...
- `UI-ACCOUNT-01`: 계정 생성과 부서 권한 부여는 독립된 성공·실패 단위를 가진다. 안전한 최초 비밀번호 설정이 실패하면 로그인 가능한 불완전 계정을 성공으로 안내하지 않는다.
...
1. 계정 최초 설정·비밀번호 재설정을 위한 만료형 token 모델과 승인된 전달 채널. 제한 CLI는 fresh DB의 최초 `SYSTEM_ADMIN` 1회 bootstrap에만 사용하며 일반 계정 설정·reset 대안으로 사용하지 않음
