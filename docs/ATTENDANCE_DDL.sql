...
CREATE UNIQUE INDEX uq_department_name_ci
    ON public.department (lower(name));
...
CREATE UNIQUE INDEX uq_account_username_ci
    ON public.account (lower(username));
...
CREATE TABLE public.device (
    ...
    CONSTRAINT ck_device_code
        CHECK (char_length(btrim(device_code)) > 0),
    ...
    CONSTRAINT ck_device_credential_test
        CHECK (
            ...
            OR
            (credential_tested_version = credential_version
...
CREATE TABLE public.attendance_record (
    ...
    CONSTRAINT fk_record_target
        ...
        ON DELETE RESTRICT,
    CONSTRAINT fk_record_band
        FOREIGN KEY (attendance_band_id)
        REFERENCES public.attendance_band (id)
        ON DELETE RESTRICT,
...
-- 5. attendance_record.attendance_band_id must belong to the policy referenced
--    by attendance_day and have parent_status equal to attendance_record.status.
-- 6. attendance_record may be created only when attendance_target.is_target is
--    true.
-- 7. An attendance_day with attendance_record rows cannot become CANCELED.
-- 8. Card status and active nfc_card_assignment must be updated atomically.
-- 9. Shared rows use this order when applicable: department, device,
...
-- 10. An administrator update to attendance status, checked-in time, or band
