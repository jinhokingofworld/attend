...
| NFR-17 | 자동 마감 | Spring 스케줄러는 매일 `Asia/Seoul` 자정에 마감 대상을 처리하고, 재기동 시 미마감 날짜를 catch-up한다. NFC 출석 저장과 날짜별 결석 생성·마감은 같은 출석 날짜 행 잠금과 유일 제약을 사용하며, 자동 마감은 하나의 멱등 트랜잭션으로 처리한다. |
