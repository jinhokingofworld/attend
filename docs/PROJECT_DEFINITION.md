...
flowchart LR
    ...
    D -->|"HTTP 요청: UID·장치 정보"| A["Spring Boot 장치 API"]
    A --> S["출석 업무 서비스"]
    J["Spring 자동 마감 작업"] --> S
    S --> M["MyBatis"]
    M --> DB[("PostgreSQL")]
    S -->|"판정 결과"| D
    D -->|"LED·부저 안내"| T
    U["관리자 웹 브라우저"] --> W["Spring Security + MVC"]
    W --> S
```
...
| 역할 | 설명 | 주요 권한 |
|---|---|---|
| 시스템 관리자 `SYSTEM_ADMIN` | 교회 전체 시스템을 관리하는 담당자 | 부서 생성, 부서 관리자 지정, 계정·장치 관리 및 전체 운영 상태 조회 |
...
MVP에서는 시스템 관리자와 부서 관리자만 웹 계정을 사용한다. 한 관리자가 여러 부서를 담당할 수 있지만, 부서 관리자는 권한이 부여된 부서의 데이터만 조회·변경할 수 있다. `USER` 역할과 교사 개인 계정은 후속 범위로 둔다.
...
| BR-03 | 출석 대상 날짜를 등록할 때 해당 부서의 활성 교사를 기본 대상자로 복사한다. 일반 대상자 조정은 정책의 태깅 시작 시각 전까지만 허용하며, 등록된 대상자 명단은 이후 소속 변경의 영향을 자동으로 받지 않는다. 단, 실제 출석한 교사가 명단에서 누락된 사실을 태깅 시작 후 발견한 경우에는 BR-21의 사후 수동 등록 절차만 예외로 허용한다. |
...
| BR-12 | 태깅 시작 시각 전 요청은 `CHECK_IN_NOT_OPEN`으로 처리한다. 마지막 지각 구간의 상한은 그 날짜의 최종 태깅 허용 시각이며 이후 요청은 `CHECK_IN_CLOSED`로 처리한다. |
...
| BR-18 | 현재 날짜보다 이전이면서 `FINALIZED` 또는 `CANCELED`가 아닌 모든 출석 대상 날짜를 자동 마감 대상으로 찾는다. |
| BR-19 | 자동 마감은 대상자 중 출석 기록이 없는 교사에게 `ABSENT`를 생성한 뒤 날짜를 `FINALIZED`로 변경한다. 결석 생성과 상태 변경은 하나의 트랜잭션으로 처리한다. |
| BR-20 | 자동 마감은 여러 번 실행해도 결과가 같아야 한다. 서버가 자정에 중지되어 있어도 재시작 또는 다음 주기 실행에서 누락된 과거 날짜를 처리한다. |
...
| BR-23 | 교사 추가·부서 제외와 부서 이동은 이후 새로 등록하는 출석 대상 날짜의 기본 명단에 반영한다. 이미 등록된 미래 날짜의 대상자는 자동 변경하지 않으며 관리자가 태깅 시작 전에 명시적으로 조정한다. 과거 대상자와 출석 기록은 자동 변경하지 않되 BR-21의 사후 수동 등록·정정은 허용한다. |
...
| BR-29 | 교사를 부서에서 제외하면 이후 새로 등록하는 출석 날짜의 기본 대상에서 빠진다. 이미 등록된 미래 날짜는 자동 변경하지 않고, 제외 확인 화면에서 태깅 시작 전 날짜를 일괄 제외할 수 있게 한다. |
...
8. 시스템은 이미 등록된 미래 출석 날짜를 보여주고, 관리자가 태깅 시작 전인 날짜의 대상자 명단에서 해당 교사를 일괄 제외할 수 있게 한다.
...
1. Spring의 자동 마감 작업이 주기적으로 현재 날짜보다 이전인 미마감 출석 대상 날짜를 찾는다.
2. 작업은 날짜 하나를 잠그고 출석 대상자 중 기록이 없는 교사에게 결석을 생성한다.
3. 대상자 수와 정상 출석·지각·결석 합계를 확인한 뒤 해당 날짜를 `FINALIZED`로 변경한다.
4. 서버가 예정 시각에 중지되어 있었다면 재시작 또는 다음 주기 실행에서 누락된 날짜를 다시 처리한다.
...
| FR-17 | 시스템은 현재 날짜보다 이전인 모든 미마감 출석 대상 날짜를 찾아 누락된 대상자를 자동 결석 처리하고 날짜를 `FINALIZED`로 변경해야 한다. | 필수 |
| FR-18 | 자동 마감은 중복 실행과 서버 재시작에도 결석 누락·중복 없이 과거 미마감 날짜를 복구해야 한다. | 필수 |
...
| 교사 등록·수정 | 최소 기본정보, 미등록 NFC 카드 선택·연결과 소속 상태 관리 | 필수 |
| 교사 상세 | 카드 상태, 날짜별 출석 이력, 기간 통계 | 필수 |
| 카드 등록함 | 자기 부서 장치의 최근 미등록 UID와 재사용 가능한 `AVAILABLE` 카드 태깅, 태깅 시각, 장치, 교사 연결·교체 | 필수 |
| 교사 부서 제외 확인 | 제외 사유, 회수·분실·폐기 카드 처리, 활성 연결 종료, 이미 등록된 미래 출석 날짜의 대상자 일괄 제외 선택 | 필수 |
...
| `Teacher` | ID, 이름, 활성 여부, 생성·수정 시각 | 이름 외 개인정보는 운영 목적이 있을 때만 선택 수집 |
...
| `AttendanceRecord` | 출석 날짜 ID, 교사 ID, 상위 상태, 정책 구간 ID·표시명, 출석 시각, 현재 판정 원천, 비고 | `(출석 날짜 ID, 교사 ID)` 유일 |
| `TagEventLog` | 요청 ID, 장치, 부서, UID, 결과, 응답, 실패 원인, 시각 | `(장치 ID, 요청 ID)` 유일, 재요청 결과 재현 |
| `AuditLog` | 부서, 작업자·시스템, 대상, 마스킹된 변경 전후 값, 사유, 시각 | 권한 있는 관리자만 조회 |

출석 관리에 필요하지 않은 생년월일과 나이는 기본 수집 대상에서 제외한다. 연락처가 실제 운영에 필요하다면 선택 항목으로 두고 수집 목적을 명확히 한다.
...
| NFR-17 | 자동 마감 | Spring 스케줄러는 주기 실행과 재기동 시 과거 미마감 날짜를 다시 찾는다. NFC 출석 저장과 날짜별 결석 생성·마감은 같은 출석 날짜 행 잠금과 유일 제약을 사용하며, 자동 마감은 하나의 멱등 트랜잭션으로 처리한다. |
...
13. 교사 추가·부서 제외, 정책 발행, 출석 대상자 변경, 카드 연결·교체·종료, 자동 결석과 출석 수동 등록·정정에는 작업자 또는 시스템 주체와 시각을 남기며 감사 로그는 권한 있는 관리자만 조회한다.
14. 백업은 운영 서버와 분리된 위치에 접근 제한하고, 개인정보가 포함되면 암호화 보관한다. 백업 실패 여부를 운영자가 확인할 수 있어야 한다.
15. 데이터와 백업의 보유기간, 탈퇴 교사의 카드 UID·태깅 로그·감사 로그 처리 방식은 교회 내부 운영 방침을 확인한 뒤 배포 전에 문서로 확정한다.
...
| AC-07 | 출석 대상 날짜에 적용 정책과 대상자 명단이 저장되며 이후 교사 추가·부서 제외·부서 이동이 과거 명단과 기록을 자동으로 바꾸지 않는다. BR-21의 사유가 있는 사후 수동 등록·정정만 명시적 예외다. |
...
| AC-27 | 부서 관리자가 사유와 카드 처리 방식을 입력해 교사를 부서에서 제외하면 활성 소속·카드 연결 종료와 카드의 `AVAILABLE`·`LOST`·`RETIRED` 전이가 같은 트랜잭션에서 처리되고 이후 새 출석 날짜의 기본 대상에서 빠진다. 과거 출석·대상자·카드 연결 이력은 유지되며, 권한 없는 다른 부서 관리자는 이 작업을 수행할 수 없다. |
...
| 자동 마감 누락·중복 또는 부분 반영 | 높음 | 과거 미마감 날짜 재검색, 행 잠금, 유일 제약과 날짜별 단일 트랜잭션 적용 |
...
| 데이터 초기화 | 앱 시작 시 테이블 DROP 및 샘플 재생성 | 운영 데이터 보존과 버전 마이그레이션 |
| 카드 등록 | 수동 편집 중심 | 미등록 태깅 목록에서 안전하게 교사 연결 |
| 오류 처리 | MVC·REST 처리 범위 중복 | 웹 오류와 API 오류 계약 분리 |
| 테스트 | 컨텍스트 로드 중심 | 도메인·DB·보안·장치 통합 자동 테스트 |
...
기본 출석 대상자는 출석 대상 날짜를 등록한 시점의 해당 부서 활성 교사로 확정한다. 일반적인 추가·제외는 해당 정책의 태깅 시작 시각 전까지만 허용한다. 이후 교사 추가·부서 제외나 부서 이동이 이미 확정된 명단을 자동으로 바꾸지는 않는다. 실제 출석자의 명단 누락은 BR-21에 따라 대상자 추가와 수동 출석 등록을 한 트랜잭션으로 처리하는 예외만 허용한다.
...
1. 교사 연락처 등 추가 개인정보가 실제 운영에 필요한지
2. 출석 이력, 태깅 로그, 감사 로그와 백업의 보유기간
3. 백업 저장 위치와 복원 시험 담당자
4. 최초 관리자와 비밀번호 재설정 책임자
