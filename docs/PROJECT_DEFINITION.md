...
| BR-20 | 자동 마감은 여러 번 실행해도 결과가 같아야 한다. 서버가 마감 시각에 중지되어 있어도 재시작 또는 다음 주기 실행에서 마감 시각이 지난 날짜를 처리한다. |
...
1. Spring의 자동 마감 작업이 주기적으로 `SCHEDULED` 상태이고 `finalization_due_at <= now`인 출석 대상 날짜를 찾는다.
...
| NFR-17 | 자동 마감 | Spring 스케줄러는 주기 실행과 재기동 시 마감 시각이 지난 미마감 날짜를 다시 찾는다. NFC 출석 저장과 날짜별 결석 생성·마감은 같은 출석 날짜 행 잠금과 유일 제약을 사용하며, 자동 마감은 하나의 멱등 트랜잭션으로 처리한다. |
