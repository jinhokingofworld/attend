...
| DB | PostgreSQL |
| 스키마 변경 | Spring SQL 초기화 차단, Flyway V001~V010 명시 실행 |
...
flowchart LR
    ...
    FLY["Flyway V001~V010<br>명시적 migration 실행"] --> TDB[("기존 member + 신규 목표 테이블")]
    LDB --- TDB
...
- fresh·정확한 레거시 DB만 허용하는 Flyway V001~V010
...
- V010 release의 runtime 검사는 history 부재, `success = FALSE` 행, V001~V010 중 누락·중복·초과 version을 모두 실패로 판정한다.
- version 문자열에 `MAX`를 사용하지 않고 Flyway `MigrationVersion`으로 해석한 적용 순서 전체를 artifact의 V001~V010 목록과 정확히 비교한다.
...
| migration test | 빈 DB V001~V010, baseline 0 레거시 fixture, 두 번 재현 |
